...
// Declarando GSAP para o TypeScript não reclamar da global
...
const Navbar: React.FC<NavbarProps> = ({ scrolled }) => {
  ...
  const getOffsetLeft = (element: HTMLElement) => {
    if (!ulRef.current || !activeElementRef.current) return 0;
    const elementRect = element.getBoundingClientRect();
    const ulRect = ulRef.current.getBoundingClientRect();

    // Calcula a posição X relativa ao elemento UL pai
    return (
      elementRect.left -
      ulRect.left +
      (elementRect.width - activeElementRef.current.offsetWidth) / 2
    );
  };
  ...
  useEffect(() => {
    // Timeout para garantir que o layout foi renderizado e as coordenadas estejam corretas
    const timer = setTimeout(() => {
      if (!ulRef.current || !activeElementRef.current || typeof gsap === 'undefined') return;

      const activeButton = ulRef.current.querySelector("li.active button") as HTMLElement;
      if (activeButton) {
  ...
  const handleNavClick = (index: number, id: string) => {
    ...
    const buttons = ulRef.current.querySelectorAll("li button");
    ...
    const oldButton = buttons[activeIndex] as HTMLElement;
    const x = getOffsetLeft(targetButton);
    const direction = index > activeIndex ? "after" : "before";
    const spacing = Math.abs(x - getOffsetLeft(oldButton));

    navRef.current?.classList.add(direction);
    ...
    gsap.to(activeElementRef.current, {
      keyframes: [
        {
          "--active-element-width": `${spacing > ulRef.current.offsetWidth - 20
            ? ulRef.current.offsetWidth - 20
            : spacing
            }px`,
          duration: 0.3,
          ease: "none",
          onStart: () => {
            if (activeElementRef.current) createSVG(activeElementRef.current);
            gsap.to(activeElementRef.current, {
        ...
        },
        {
          ...
          onComplete: () => {
            if (activeElementRef.current) {
              ...
              navRef.current?.classList.remove("before", "after");
              activeElementRef.current.removeAttribute("style");
              gsap.set(activeElementRef.current, {
                x: getOffsetLeft(targetButton),
                "--active-element-show": "1",
              });
            }
          },
  ...
  return (
    <nav 
      ref={navRef}
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${scrolled ? 'bg-black/80 backdrop-blur-xl py-3 border-b border-white/5 shadow-2xl' : 'bg-transparent py-5'}`}
    >
      <div className="container mx-auto px-4 flex justify-between items-center max-w-7xl relative">
        
        <div 
          onClick={() => handleNavClick(0, 'top')}
          ...
          {menuItems.map((item, idx) => (
            <li key={item.id} className={activeIndex === idx ? 'active' : ''}>
              <button 
                ...
                className={`transition-all uppercase py-2 ${activeIndex === idx ? 'text-primary' : 'text-slate-400 hover:text-white'}`}
              >
                {item.label}
              </button>
            </li>
          ))}
          {/* O Elemento Ativo que viaja - Agora relativo ao UL */}
